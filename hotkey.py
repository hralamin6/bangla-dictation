...
async def monitor_device(device, loop):
    ...
    async def trigger_start():
        nonlocal is_recording, start_task
        await asyncio.sleep(0.15) # Wait 150ms to see if Alt is also pressed
        if r_ctrl_pressed and r_alt_pressed:
            is_recording = True
    ...
    try:
        async for event in device.async_read_loop():
            if event.type == ecodes.EV_KEY:
                ...
                if r_ctrl_pressed and not is_recording and start_task is None:
